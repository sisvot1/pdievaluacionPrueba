...
if (!empty($testpostgres)) {
	...
	if ($db->Connect("mysqlean.ean.edu.co","tester","test","test")) {
...
if (!empty($testibase)) {
	...
	print "<h1>Connecting $db->databaseType...</h1>";
	if ($db->PConnect("mysqlean.ean.edu.co:d:\\firebird\\151\\examples\\EMPLOYEE.fdb", "sysdba", "masterkey", ""))
...
if (!empty($testvfp)) { // ODBC
	...
	if ( $db->PConnect('mysqlean.ean.edu.co','DRIVER={Microsoft Visual FoxPro Driver};SOURCETYPE=DBF;SOURCEDB=d:\inetpub\adodb;EXCLUSIVE=NO;')) {
	print "<h1>Connecting $db->databaseType...</h1>";flush();
	testdb($db,"create table d:\\inetpub\\adodb\\ADOXYZ (id int, firstname char(24), lastname char(24),created date)");
...
// REQUIRES MySQL server at mysqlean.ean.edu.co with database 'test'
if (!empty($testmysql)) { // MYSQL


	if (PHP_VERSION >= 5 || $_SERVER['HTTP_HOST'] == 'mysqlean.ean.edu.co') $server = 'mysqlean.ean.edu.co';
	else $server = "mangrove";
	$user = 'dbmanager'; $password = 'kr56f10qh'; $database = 'northwind';
	...
	if (true || $db->PConnect($server, "root", "", "northwind")) {
		...
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) Type=InnoDB");
	} else print "ERROR: MySQL test requires a MySQL server on mysqlean.ean.edu.co, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
}
...
if (!empty($testmysqlodbc)) { // MYSQL
	...
	$db->hasTransactions = false;
	print "<h1>Connecting $db->databaseType...</h1>";
	if ($_SERVER['HTTP_HOST'] == 'mysqlean.ean.edu.co') $server = 'mysqlean.ean.edu.co';
	else $server = "mangrove";
...
if (!empty($testproxy)){
	...
	if ($db->PConnect('http://mysqlean.ean.edu.co/php/phplens/adodb/server.php'))
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) type=innodb");
	else print "ERROR: MySQL test requires a MySQL server on mysqlean.ean.edu.co, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testdb2)) { 
	...
	$dsn = "db2_sample";
	$dsn = "driver={IBM db2 odbc DRIVER};Database=sample;hostname=mysqlean.ean.edu.co;port=50000;protocol=TCPIP; uid=root; pwd=natsoft";
	if ($db->Connect($dsn)) {
	//	testdb($db,"create table ADOXYZ (id int, firstname varchar(24), lastname varchar(24),created date)");
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL with OLEDB provider
	...
	//$db->debug=1;
	$myDSN="SERVER=mysqlean.ean.edu.co;DATABASE=northwind;Trusted_Connection=yes";
...
if (extension_loaded('odbtp') && !empty($testmssql)) { // MS SQL Server via ODBC
	...
	if ($db->PConnect('mysqlean.ean.edu.co',$dsn, "", ""))  {
		print "<h1>Connecting $db->databaseType...</h1>";				
		testdb($db,"create table ADOXYZ (id int, firstname char(24) null, lastname char(24) null,created datetime null)");
	}
	else print "ERROR: MSSQL test 1 requires a MS SQL 7 server setup with DSN setup";
