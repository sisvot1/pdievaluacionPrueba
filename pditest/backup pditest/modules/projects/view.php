...
if ($hasTasks) { 
    ...
    $q->addQuery("company_name, CONCAT_WS(', ',contact_last_name,contact_first_name) user_name, projects.*,"
                 ." SUM(t1.task_percent_complete)/Count(t1.task_percent_complete)"
...
$progreso_objetivo = sprintf("%.1f%%", $obj->project_percent_complete); 
...
$titleBlock->addCrumb( "?m=projects&a=reports&project_company=$project_company&project_id=$project_id", "reports" );
$titleBlock->show();
?>
		...
		<tr>    
			...
			<?php if ($perms->checkModuleItem( 'companies', 'access', $obj->project_company )) {?>
            			<td class="hilite" width="100%"> <?php echo "<a href='?m=companies&a=view&company_id=" . $obj->project_company ."'>" . htmlspecialchars( $obj->company_name, ENT_QUOTES) . '</a>' ;?></td>
                                <?php echo htmlspecialchars( $obj->company_name, ENT_QUOTES) ;?>
			<?php } else {?>
	...
	<td width="50%" rowspan="9" valign="top">
		...
		<tr>
			...
			<td class="hilite" width="100%"><?php echo $AppUI->_($pstatus[$obj->project_status]);?></td>
                    item a seleccionar    <?php echo $AppUI->_($pstatus[$obj->project_status]);?>
		...
		<tr>
			...
			<td class="hilite" width="100%"><?php printf( "%.1f%%", $obj->project_percent_complete );?></td>
                        <?php printf( "%.1f%%", $obj->project_percent_complete );?>
...
<?php
$tabBox = new CTabBox( "?m=projects&a=view&project_id=$project_id&project_company=sproject_company", "", $tab );
$query_string = "?m=projects&a=view&project_id=$project_id&project_company=$project_company";
