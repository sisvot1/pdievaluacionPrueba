...
function getAllowedcompaniesInRows($userId) {
		$q = new DBQuery;
		$q->addQuery('company_id', 'company_module', 'company_name', 'company_description');
		$q->addTable('companies');                     
		$q->addOrder('company_description');
		$this->setAllowedSQL($userId, $q);
		$allowedCompaniesRows = $q->exec();
		
		return $allowedcompaniesRows;
	}
