...
echo "ss".$obj->task_id;
        ...
        echo "<strong>Avance</strong>"." ".$obj->task_percent_complete."|";
        echo "<strong>Meta</strong>"." ".$obj->task_name."|";
        echo "<strong>+</strong>"." ".$obj->task_project."|";
        echo "<strong>Objetivo</strong>"." ".$obj->project_name."|";
        echo "<strong>+</strong>"." ".$obj->task_parent."|";
        echo "<strong>departamento</strong>"." ".$obj->task_departments."|";
        echo $nombre_compania = $_REQUEST["nombre_compania"]."<br />"; 
        echo $estado_proyecto = $_REQUEST["estado_proyecto"]."<br />"; 
        echo "id_proyecto".$obj->task_project."<br />"; 
...
<?php
/*$task_percent_complete = $obj->task_percent_complete;
$task_name = $obj->task_name;
$project_name = $obj->project_name;
global $task_percent_complete, $task_name, $project_name, $task_project;

$task_project = $obj->task_project;
require_once("modules/tasks/consulta_tarea/class/class.php");
$obj5=new Consulta();
$dep1=$obj5->consulta_proceso($task_id); 
$nombre_vigencia1=$obj5->nombre_vigencia();
$porcentaje_objetivo=$obj5->porcentaje_objetivo($task_project);
$dimension=$obj5->dimension($task_project);
    
if (isset($_POST["grabar"]) and $_POST["grabar"]=="si")
{
	//print_r($_POST);
	$obj5->insertar_historicos();
	exit;
}*/
...
document.getElementById("valor").innerHTML="<font color='#ff0000'>.::Histórico Almacenado Satisfactoriamente::.</font>"; }</script>
<noscript>
...
<table border="0" cellpadding="4" cellspacing="0" width="100%" class="std">
      <br />  <center>           
        ...
        <input type="hidden" name="porcentaje" value="<?php echo @$obj->task_percent_complete;?>%" /> 
        <input type="hidden" name="meta" value="<?php echo $obj->task_name;?>" /> 
        <input type="hidden" name="proyecto" value="<?php echo $obj->project_name;?>" />     
        <input type="hidden" name="proceso" value="<?php
              for ($i=0;$i<count($dep1);$i++)
              {
                 
                 $dep2= $dep1[$i]["dept_name"];//nombre proceso     
                  $proceso_con = ucfirst(strtolower(str_replace("Ã","í",$dep2))); 
                 echo str_replace("Ã“","ó",$proceso_con);
                 
              }
              ?>" />      
        <input type="hidden" name="task_id" value="<?php echo $obj->task_id;?>" />    
            
        <input type="hidden" name="nombre_compania" value="<?php
              for ($i=0;$i<count($nombre_vigencia1);$i++)
              {
               $nombre_vigencia2 = $nombre_vigencia1[$i]["vigencia"];//nombre vigencia    
               echo ucfirst(strtolower(str_replace("Ã³","ó",$nombre_vigencia2)));
              }
              ?>" />    
        
       <input type="hidden" name="progreso_objetivo" value="<?php
              for ($i=0;$i<count($porcentaje_objetivo);$i++)
              {
              		$porcen = $porcentaje_objetivo[$i] ["porcentaje_objetivo"];
                 echo substr($porcen,0, 5)."%";
              }
              ?>
               " /> 
       
              
       <input type="hidden" name="estado_proyecto" value="<?php
              for ($i=0;$i<count($dimension);$i++)
              {
              		$dim = $dimension[$i]["project_status"];
              		if ($dim == 0)
              		{
              			echo "Estratégico";
              		}
	              		elseif ($dim == 1)
	              		{
	              			echo "Mejoramiento (15)";
	              		}
		              		elseif ($dim == 2)
		              		{
		              			echo "Compromiso terceros (13)";
        ...
        <input type="hidden" name="id_proyecto" value="'.$obj->task_project.'" />         
