...
$companies_list=array("0"=> $AppUI->_("All", UI_OUTPUT_RAW) );

$obj = new CProject();
$ptrc = $obj->getAllowedProjectsInRows($AppUI->user_id);
$ptrc1 = $obj->getAllowedProjectsInRows($AppUI->user_id);

$nums=db_num_rows($ptrc);
$nums1=db_num_rows($ptrc1);

echo db_error();
for ($y=0; $y < $nums1; $y++) {
	$row = db_fetch_assoc( $ptrc1);
	if ($row["company_id"] == $company_id) $display_company_name='('.$row["company_name"].') '.$row["company_name"];
	$companies_list[$row["company_id"]] = '('.$row["company_name"].') '.$row["company_name"];
}
...
echo $AppUI->_('Vigencia Seleccionada') . ": <b>".$display_company_id."</b>"; 
...
<?php echo arraySelect( $companies_list, 'project_company', 'size="1" class="text" onchange="changeIt();"', $company_id, false );?>
