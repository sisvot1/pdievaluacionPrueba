...
if ($do_report) {
	...
	if ($project_id==0) {
        echo "<tr><th>Objetivo</th><th>Nombre de la Meta";} else {echo "<th>Meta</th>";}
	echo "<th width=200>Descripcion de la Meta</th>";
	echo "<th>Asignado a</th>";
        echo "<th>Proceso</th>";
        echo "<th>Fecha de Inicio de la Meta</th>";
	echo "<th>Fecha Final de la Meta</th>";
	echo "<th>% Cumplimiento</th></tr>";

	$pdfdata = array();
	$columns = array(
        "<b>".$AppUI->_('Task Name')."</b>",
	"<b>".$AppUI->_('Descripcion de la Meta')."</b>",
	"<b>".$AppUI->_('Asignado a')."</b>",
        "<b>".$AppUI->_('Proceso')."</b>",
	"<b>".$AppUI->_('Fecha de Inicio de la meta')."</b>",
	"<b>".$AppUI->_('Fecha Final de la Meta')."</b>",
	"<b>".$AppUI->_('% de Cumplimiento')."</b>"
	);
	if ($project_id==0)
	...
	while  ($Tasks = db_fetch_assoc($Task_List)){
		...
		while ($Task_User = db_fetch_assoc($sql_user)){
			...
			$users .= $user_list['contact_first_name']." ".$user_list['contact_last_name'];
                        $users .= $user_list['contact_department'];


		}
		$str =  "<tr>";
		if ($project_id==0){$str .= "<td>".$Tasks['dept_name']."</td>";}
...
if ($log_pdf) {
	// make the PDF file
		...
		$pdf->ezText( "\n" . $AppUI->_('Reporte de Metas por Objetivo'), 12 );
		if ($project_id != 0) {$pdf->ezText( $pname, 15 );}
		if ($log_all) {
			$pdf->ezText( "Todas las Metas Ingresadas", 9 );
		} else {
