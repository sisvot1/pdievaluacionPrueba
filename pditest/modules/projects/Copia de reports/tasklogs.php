...
<tr>
	...
	<?php
		...
		FROM users LEFT JOIN contacts ON user_contact = contact_id";
...
if ($do_report) {
	...
	$sql = "SELECT p.project_id, p.project_name, t.*, CONCAT_WS(' ',contact_first_name,contact_last_name) AS creator, " 
		...
		."\nWHERE task_log_task = task_id ORDER BY project_name";
	if ($project_id != 0) {
...
<?php
	...
	// make the PDF file
		if ($project_id != 0) {
			$sql = "SELECT project_name FROM projects WHERE project_id=$project_id order by project_name";
		...
		$title = 'Listado de Objetivos y Metas';
